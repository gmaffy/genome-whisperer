...
func validateGvcf(vcf string, verbose bool) error {
	valStr := fmt.Sprintf(`gunzip -t %s && bcftools stats -r "" %s > /dev/null`, vcf, vcf)
	if verbose {
...
func VariantCallingDir(dataDir string, species string, refVer string, refFasta string, caller string, merger string, dvVer string, modelType string, verbose bool, noMerging bool, gatkLogLevel string) {
	...
	// Deduplicate sample names while preserving order.
	...
	fmt.Printf("Found %d sample(s) in the data directory for %s\n==================================\n\n", len(samples), species)
	...
	fmt.Printf("Valid: %d  Missing: %d  Multiple: %d\n==================================\n\n",
		len(validSamples), len(missingBams), len(multipleBams))

	if len(validSamples) == 0 {
		fmt.Println("No valid samples found. Exiting.")
		return
	}
	...
	for i := 0; i < maxWorkers; i++ {
		...
		go func() {
			...
			for item := range workCh {
				...
				// Single lookup: get existing gVCF files for this (sample, chrom).
				_, vcfFiles := findBamOrVcfs(dataDirAbs, species, sw.sample, refVer, "gvcfs", chrom.ID)

				switch len(vcfFiles) {
				case 0:
					fmt.Printf("[%s] gVCF not found for chrom %s — creating\n", sw.sample, chrom.ID)
					if _, err := CreateGvcf(sw.cram, refFasta, []SeqInfo{chrom}, gvcfPath, gatkLogLevel, caller, dvVer, modelType, verbose); err != nil {
						fmt.Printf("[%s] Error creating gVCF for chrom %s: %v\n", sw.sample, chrom.ID, err)
					}

				case 1:
					vcf := vcfFiles[0]
					color.Green("[%s] Validating existing gVCF for chrom %s: %s\n", sw.sample, chrom.ID, color.BlueString(vcf))
					if vErr := validateGvcf(vcf, verbose); vErr != nil {
						fmt.Printf("[%s] gVCF invalid for chrom %s (%v) — re-creating\n", sw.sample, chrom.ID, vErr)
						if _, err := CreateGvcf(sw.cram, refFasta, []SeqInfo{chrom}, gvcfPath, gatkLogLevel, caller, dvVer, modelType, verbose); err != nil {
							fmt.Printf("[%s] Error re-creating gVCF for chrom %s: %v\n", sw.sample, chrom.ID, err)
						}
					} else {
						fmt.Printf("[%s] gVCF valid for chrom %s\n", sw.sample, chrom.ID)
					}

				default:
					fmt.Printf("[%s] Multiple gVCF files found for chrom %s — skipping\n", sw.sample, chrom.ID)
				}
			}
		}()
	}
	...
	for _, sw := range validSamples {
		...
		if _, statErr := os.Stat(contigGvcfPath); statErr == nil {
			fmt.Printf("[%s] Contig gVCF already exists — skipping: %s\n", sw.sample, contigGvcfPath)
			continue
		}

		fmt.Printf("[%s] Creating contig gVCF: %s\n", sw.sample, contigGvcfPath)
