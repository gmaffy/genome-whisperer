...
func FindBamOrVcfs(dataDirAbs string, species string, sample string, refVer string, bamOrVcf string, chrom string) (error, []string) {
	...
	if err != nil {
		return fmt.Errorf("glob error: %w", err), nil
	}
	if len(matches) == 0 {
		return fmt.Errorf("no %s files found in %s", bamOrVcf, sample), nil
	}
	if len(matches) > 1 {
...
//
// FIX: DeepVariant --intermediate_results_dir is now unique per (bam, region)
// to prevent a FileExistsError race when multiple goroutines share the same
// output mount point.
// ─────────────────────────────────────────────────────────────────────────────
...
func CreateGvcf(bam string, refFile string, chroms []SeqInfo, theGVCF string, gatkLogLevel string, caller string, dvVer string, modelType string, verbose bool) (string, error) {
	...
	case "gatk":
		if len(chroms) == 1 {
			...
			listPath := filepath.Join(os.TempDir(), "gatk_intervals_contigs.list")
	...
	case "deepvariant":
		if len(chroms) == 1 {
			...
			listPath := filepath.Join(os.TempDir(), "deepvariant_intervals_contigs.list")
			f, err := os.Create(listPath)
			if err != nil {
		...
		// FIX: make intermediate_results_dir unique per (sample, region) so
		// concurrent docker containers don't race on mkdir /output/tmp.
		safeRegion := strings.ReplaceAll(regionArg, string(os.PathSeparator), "_")
		safeRegion = strings.ReplaceAll(safeRegion, ".", "_")
...
// ─────────────────────────────────────────────────────────────────────────────
// VariantCalling is the core pipeline: per-chromosome gVCF → joint VCF →
// hard-filtered SNPs + INDELs → per-chromosome hard-filtered VCF →
// final merged multi-sample VCF.
//
// FIX: replaced every log.Fatalf / os.Exit inside goroutines with proper
// error propagation via a channel so that all goroutines can finish cleanly
// (defers run) and the main goroutine decides what to do.
// ─────────────────────────────────────────────────────────────────────────────
...
func VariantCalling(
	...
	var (
		wg          sync.WaitGroup
		glnxMu      sync.Mutex // serialises GLnexus (writes to CWD)
	...
	for sc.Next() {
		...
		case "gatk":
			go func(seq *linear.Seq,
				gvcfPath, tmpPath, vcfPath, theDB string,
				jointVCF, jointBCF, snpVCF, indelVCF string,
				hardFilteredSNPs, hardFilteredINDELs, hardFilteredVCF string,
			) {
				defer func() { wg.Done(); <-sem }()

				slog.Info(fmt.Sprintf("Working on chromosome %s …\n", seq.ID))

				// ── HaplotypeCaller (all BAMs in parallel) ────────────────────
				var (
				...
				// FIX: propagate HaplotypeCaller errors upward
				if callerErr != nil {
				...
				case "glnexus":
					if !utils.StageHasCompleted(logged, stageGLNEXUS, "ALL", seq.ID) {
						// FIX: GLnexus writes "GLnexus.DB" into CWD; serialise with mutex
						// AND check/remove the DB *inside* the lock so no two goroutines
						// race on the same directory.
						glnxMu.Lock()
						glnexusCmd := fmt.Sprintf(`glnexus_cli --config gatk %s/*.vcf.gz > %s`, gvcfPath, jointBCF)
						...
						_ = os.RemoveAll(glnexusDBDir) // pre-clean; ignore error
						glErr := runCmd(glnexusCmd, verbose)
						_ = os.RemoveAll(glnexusDBDir) // post-clean
						glnxMu.Unlock()
				...
				// ── hard filtering ────────────────────────────────────────────
				snpVCF, indelVCF, hardFilteredSNPs, hardFilteredINDELs, err :=
			...
			}(seq, gvcfPath, tmpPath, vcfPath, theDB,
				jointVCF, jointBCF, snpVCF, indelVCF,
				hardFilteredSNPs, hardFilteredINDELs, hardFilteredVCF)
		...
		case "deepvariant":
			...
			go func(seq *linear.Seq,
				gvcfPath, vcfPath string,
				jointVCF, jointBCF, snpVCF, indelVCF string,
				hardFilteredSNPs, hardFilteredINDELs, hardFilteredVCF string,
			) {
				defer func() { wg.Done(); <-sem }()

				// ── per-BAM DeepVariant calls in parallel ─────────────────────
				var (
					callerWg  sync.WaitGroup
					callerErr error
					callerMu  sync.Mutex
				...
				for _, bam := range bams {
					...
					callerWg.Add(1)
					go func(bam, bamName, bamDir, refDir, refName, vcfName, gvcfName string) {
						defer callerWg.Done()
						...
						// FIX: unique intermediate dir per (bam, chrom) eliminates
						// the FileExistsError race between concurrent containers.
						intermediateName := fmt.Sprintf("tmp_%s_%s",
							strings.TrimSuffix(bamName, filepath.Ext(bamName)),
							chromDir,
						)
						...
						dvCmdStr := fmt.Sprintf(
							`docker run -v "%s":"/bam" -v "%s":"/ref" -v "%s":"/output" google/deepvariant:"%s" `+
								...
								`--regions "%s" --output_vcf=/output/%s --output_gvcf=/output/%s `+
								`--intermediate_results_dir /output/%s`,
							bamDir, refDir, gvcfPath, dvVer,
							modelType, refName, bamName,
							seq.ID, vcfName, gvcfName,
							intermediateName,
						)

						if dvErr := runCmd(dvCmdStr, verbose); dvErr != nil {
				...
				if !utils.StageHasCompleted(logged, stageGLNEXUS, "ALL", seq.ID) {
					glnxMu.Lock()
					glnexusCmd := fmt.Sprintf(`glnexus_cli --config %s %s/*.vcf.gz > %s`, caller, gvcfPath, jointBCF)
					jlog.Info("VARIANT CALLING", "PROGRAM", stageGLNEXUS, "SAMPLE", "ALL", "CHROMOSOME", seq.ID, "STATUS", "STARTED", "CMD", glnexusCmd)
					slog.Info("VARIANT CALLING", "PROGRAM", stageGLNEXUS, "SAMPLE", "ALL", "CHROMOSOME", seq.ID, "STATUS", "STARTED")
					_ = os.RemoveAll(glnexusDBDir)
					glErr := runCmd(glnexusCmd, verbose)
					_ = os.RemoveAll(glnexusDBDir)
					glnxMu.Unlock()
			...
			}(seq, gvcfPath, vcfPath,
				jointVCF, jointBCF, snpVCF, indelVCF,
				hardFilteredSNPs, hardFilteredINDELs, hardFilteredVCF)

		default:
			// FIX: unknown caller — release semaphore and return an error instead
			// of os.Exit(1) inside the loop.
			wg.Done()
			<-sem
			return "", fmt.Errorf("unknown caller %q: must be \"gatk\" or \"deepvariant\"", caller)
		}
	...
	// Collect any errors reported by goroutines.
...
func runHardFiltering(
	chromID string,
	jointVCF, snpVCF, indelVCF, hardFilteredSNPs, hardFilteredINDELs string,
	gatkLogLevel string,
	verbose bool,
	logged []utils.LogEntry,
	jlog *slog.Logger,
...
func mergeHardFiltered(
	chromID, hardFilteredSNPs, hardFilteredINDELs, hardFilteredVCF string,
	gatkLogLevel string,
	verbose bool,
	logged []utils.LogEntry,
	jlog *slog.Logger,
