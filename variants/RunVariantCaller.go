...
func MergeGvcfsGATK(gvcfs []string, refFile string, chrom string, species string, refVer string, verbose bool, logLevel string, outDir string, logged []utils.LogEntry, jlog *slog.Logger) (string, error) {
	...
	if !utils.StageHasCompleted(logged, stageGenomicsDBImport, "ALL", chrom) {
		...
		if err := runCmd(gDBCmd, verbose); err != nil {
	...
	if !utils.StageHasCompleted(logged, stageGenotypeGVCFs, "ALL", chrom) {
		...
		if err := runCmd(genoCmd, verbose); err != nil {
...
func MergeGvcfsGlnexus(gvcfs []string, chrom string, species string, refVer string, caller string, verbose bool, outDir string, logged []utils.LogEntry, jlog *slog.Logger) (string, error) {
	...
	if !utils.StageHasCompleted(logged, stageGLNEXUS, "ALL", chrom) {
		...
		if err := runCmd(glnexusCmd, verbose); err != nil {
	...
	if !utils.StageHasCompleted(logged, stageGLNEXUSBCFTOOLS, "ALL", chrom) {
		...
		if err := runCmd(bcfCmd, verbose); err != nil {
	...
	if !utils.StageHasCompleted(logged, stageGLNEXUSIndex, "ALL", chrom) {
		...
		if err := runCmd(indexCmd, verbose); err != nil {
	...
	case "gatk":
		...
		for _, chrom := range chroms {
			...
			for _, bam := range bams {
				...
				if utils.StageHasCompleted(logged, stageHaplotypeCaller, bamName, chrom.ID) {
					...
					if err != nil {
						return "", fmt.Errorf("resolving existing gVCF path for %s / %s: %w", bamName, chrom.ID, err)
					}
					gvcfs = append(gvcfs, chromGVCF)
					continue
				}

				jlog.Info("VARIANT CALLING", "PROGRAM", stageHaplotypeCaller, "SAMPLE", bamName, "CHROMOSOME", chrom.ID, "STATUS", "STARTED")
				slog.Info("VARIANT CALLING", "PROGRAM", stageHaplotypeCaller, "SAMPLE", bamName, "CHROMOSOME", chrom.ID, "STATUS", "STARTED")

				chromGVCF, err := CreateGvcfGATK(bam, refFile, []SeqInfo{chrom}, species, refVer, gatkLogLevel, verbose, out)
				if err != nil {
					jlog.Error("VARIANT CALLING", "PROGRAM", stageHaplotypeCaller, "SAMPLE", bamName, "CHROMOSOME", chrom.ID, "STATUS", fmt.Sprintf("FAILED: %v", err))
					slog.Error("VARIANT CALLING", "PROGRAM", stageHaplotypeCaller, "SAMPLE", bamName, "CHROMOSOME", chrom.ID, "STATUS", fmt.Sprintf("FAILED: %v", err))
					return "", fmt.Errorf("creating gVCF for %s with GATK HaplotypeCaller: %w", chrom.ID, err)
				}
				jlog.Info("VARIANT CALLING", "PROGRAM", stageHaplotypeCaller, "SAMPLE", bamName, "CHROMOSOME", chrom.ID, "STATUS", "COMPLETED")
				slog.Info("VARIANT CALLING", "PROGRAM", stageHaplotypeCaller, "SAMPLE", bamName, "CHROMOSOME", chrom.ID, "STATUS", "COMPLETED")
