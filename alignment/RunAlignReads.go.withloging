...
func RunAlignReadsDir(dataDir string, species string, refVer string, refFasta string, genomesDir string, verbose bool, gatkLogLevel string, aligner string, quick bool, skipVer bool, bqsr bool, bootstrap bool, knownSites []string, threads int) {
	...
	// ============================================= Log file ================================================ //
	logFilePath := filepath.Join(dataDir, "alignment_dir.log")
	logFile, logOpenErr := os.OpenFile(logFilePath, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if logOpenErr != nil {
		log.Fatalf("Failed to open log file: %v", logOpenErr)
	}
	defer logFile.Close()
	jsonHandler := slog.NewJSONHandler(logFile, nil)
	jlog := slog.New(jsonHandler)
	...
	for _, sample := range samples {
		...
		case 1:
			...
			if err := variants.ValidateBam(cram, resolvedFasta, verbose, quick); err != nil {
				...
				validSamples = append(validSamples, sampleWork{
					sample:  sample,
					cram:    cram,
					cramDir: filepath.Dir(filepath.Dir(filepath.Dir(cram))),
				})
			}
		default:
			color.Red("[%s] has multiple cram files ⚠️. Remove extras: %v\n", sample, cramFiles)
	...
	color.Green("\nReady to align: %d\n", len(validCleanReads))
	color.Red("Could not prepare (missing/invalid reads): %d\n", len(missingCleanReads))
	...
	for _, rp := range validCleanReads {
		...
		go func(readPair samplePair) {
			...
			sampleLogPath := filepath.Join(readPair.bamDir, sampleName+".alignment.log")
			...
			// ---- Open per-sample log ---- //
			sf, sfErr := os.OpenFile(sampleLogPath, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
			if sfErr != nil {
				addFailure(fmt.Sprintf("open log: %v", sfErr))
				return
			}
			defer sf.Close()
			sl := slog.New(slog.NewJSONHandler(sf, nil))
			slogged := utils.ParseLogFile(sampleLogPath)
			...
			if aligner == "bwa-mem" {
				alignerLabel = "BWA_MEM"
				alignCmd = fmt.Sprintf(
					`bwa mem -t %v -M -Y -R '@RG\tID:%s.1\tSM:%s\tLB:%s_1\tPL:BGISEQ' %s %s %s | samtools sort -o %s`,
					threads, sampleName, sampleName, sampleName, resolvedFasta, readPair.fwd, readPair.rev, sortedBam)
			}

			if utils.StageHasCompleted(slogged, alignerLabel, sampleName, "ALL") {
				color.Yellow("[%s] %s already completed. Skipping.\n", sampleName, alignerLabel)
			} else {
				sl.Info("ALIGNMENT", "PROGRAM", alignerLabel, "SAMPLE", sampleName, "STATUS", "STARTED")
				jlog.Info("ALIGNMENT", "PROGRAM", alignerLabel, "SAMPLE", sampleName, "STATUS", "STARTED")
				fmt.Printf("[%s] %s\n", sampleName, alignCmd)
				...
				if memErr != nil {
					sl.Error("ALIGNMENT", "PROGRAM", alignerLabel, "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", memErr))
					jlog.Error("ALIGNMENT", "PROGRAM", alignerLabel, "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", memErr))
					addFailure(fmt.Sprintf("%s: %v", alignerLabel, memErr))
					return
				}
				sl.Info("ALIGNMENT", "PROGRAM", alignerLabel, "SAMPLE", sampleName, "STATUS", "COMPLETED")
				jlog.Info("ALIGNMENT", "PROGRAM", alignerLabel, "SAMPLE", sampleName, "STATUS", "COMPLETED")
				color.Green("[%s] %s completed.\n", sampleName, alignerLabel)
			}

			// ----------------------------------------- Mark Duplicates -------------------------------------------- //
			if utils.StageHasCompleted(slogged, "MARK_DUPLICATES", sampleName, "ALL") {
				color.Yellow("[%s] MARK_DUPLICATES already completed. Skipping.\n", sampleName)
			} else {
				sl.Info("ALIGNMENT", "PROGRAM", "MARK_DUPLICATES", "SAMPLE", sampleName, "STATUS", "STARTED")
				jlog.Info("ALIGNMENT", "PROGRAM", "MARK_DUPLICATES", "SAMPLE", sampleName, "STATUS", "STARTED")
				...
				if mdupErr != nil {
					sl.Error("ALIGNMENT", "PROGRAM", "MARK_DUPLICATES", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", mdupErr))
					jlog.Error("ALIGNMENT", "PROGRAM", "MARK_DUPLICATES", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", mdupErr))
					addFailure(fmt.Sprintf("MARK_DUPLICATES: %v", mdupErr))
					return
				}
				sl.Info("ALIGNMENT", "PROGRAM", "MARK_DUPLICATES", "SAMPLE", sampleName, "STATUS", "COMPLETED")
				jlog.Info("ALIGNMENT", "PROGRAM", "MARK_DUPLICATES", "SAMPLE", sampleName, "STATUS", "COMPLETED")
				color.Green("[%s] MARK_DUPLICATES completed.\n", sampleName)
			}

			// ----------------------------------------- BAM Index -------------------------------------------------- //
			if utils.StageHasCompleted(slogged, "BAM_INDEX", sampleName, "ALL") {
				color.Yellow("[%s] BAM_INDEX already completed. Skipping.\n", sampleName)
			} else {
				sl.Info("ALIGNMENT", "PROGRAM", "BAM_INDEX", "SAMPLE", sampleName, "STATUS", "STARTED")
				jlog.Info("ALIGNMENT", "PROGRAM", "BAM_INDEX", "SAMPLE", sampleName, "STATUS", "STARTED")
				...
				if indErr != nil {
					sl.Error("ALIGNMENT", "PROGRAM", "BAM_INDEX", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", indErr))
					jlog.Error("ALIGNMENT", "PROGRAM", "BAM_INDEX", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", indErr))
					addFailure(fmt.Sprintf("BAM_INDEX: %v", indErr))
					return
				}
				sl.Info("ALIGNMENT", "PROGRAM", "BAM_INDEX", "SAMPLE", sampleName, "STATUS", "COMPLETED")
				jlog.Info("ALIGNMENT", "PROGRAM", "BAM_INDEX", "SAMPLE", sampleName, "STATUS", "COMPLETED")
				color.Green("[%s] BAM_INDEX completed.\n", sampleName)
			}

			// ============================================= BQSR ==================================================== //
			if !bqsr {
				color.Green("[%s] Pipeline complete (no BQSR). Final BAM: %s\n", sampleName, rgmdBam)
				return
			}

			color.Green("[%s] Starting BQSR...\n", sampleName)

			// Per-sample copy of knownSites so we can append bootstrap results without races
			sampleKnownSites := make([]string, len(knownSites))
			copy(sampleKnownSites, knownSites)

			// ---- Bootstrap: create known variants from the rgmd BAM ---- //
			if len(sampleKnownSites) == 0 && bootstrap {
				if utils.StageHasCompleted(slogged, "BOOTSTRAP_CKV", sampleName, "ALL") {
					color.Yellow("[%s] BOOTSTRAP_CKV already completed. Reusing known-sites.\n", sampleName)
					// Derive expected VCF paths from rgmdBam stem
					stem := strings.TrimSuffix(strings.TrimSuffix(rgmdBam, ".bam"), ".cram")
					snpVCF := stem + ".raw.SNP.hard_filtered.vcf.gz"
					indelVCF := stem + ".raw.INDEL.hard_filtered.vcf.gz"
					for _, vcf := range []string{snpVCF, indelVCF} {
						if _, statErr := os.Stat(vcf); statErr == nil {
							sampleKnownSites = append(sampleKnownSites, vcf)
						}
					}
					// If VCFs have disappeared, re-run
					if len(sampleKnownSites) == 0 {
						sl.Info("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", "RETRYING_CREATE_KNOWN_VARIANTS")
						jlog.Info("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", "RETRYING_CREATE_KNOWN_VARIANTS")
						newKS, ksErr := CreateKnownVariants(resolvedFasta, rgmdBam, sampleLogPath, gatkLogLevel, verbose)
						if ksErr != nil {
							sl.Error("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", ksErr))
							jlog.Error("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", ksErr))
							addFailure(fmt.Sprintf("BOOTSTRAP_CKV (retry): %v", ksErr))
							return
						}
						sampleKnownSites = newKS
					}
				} else {
					sl.Info("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", "STARTED")
					jlog.Info("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", "STARTED")
					newKS, ksErr := CreateKnownVariants(resolvedFasta, rgmdBam, sampleLogPath, gatkLogLevel, verbose)
					if ksErr != nil {
						sl.Error("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", ksErr))
						jlog.Error("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", ksErr))
						addFailure(fmt.Sprintf("BOOTSTRAP_CKV: %v", ksErr))
						return
					}
					sl.Info("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", "COMPLETED")
					jlog.Info("BQSR", "PROGRAM", "BOOTSTRAP_CKV", "SAMPLE", sampleName, "STATUS", "COMPLETED")
					sampleKnownSites = newKS
					color.Green("[%s] Bootstrap known variants: %v\n", sampleName, sampleKnownSites)
				}
			}

			// ---- Recalibrate ---- //
			bqsrStage := "DBSNP_BQSR"
			if bootstrap {
				bqsrStage = "BOOTSTRAP_BQSR"
			}

			if utils.StageHasCompleted(slogged, bqsrStage, sampleName, "ALL") {
				color.Yellow("[%s] %s already completed. Skipping.\n", sampleName, bqsrStage)
				return
			}

			sl.Info("BQSR", "PROGRAM", bqsrStage, "SAMPLE", sampleName, "STATUS", "STARTED")
			jlog.Info("BQSR", "PROGRAM", bqsrStage, "SAMPLE", sampleName, "STATUS", "STARTED")

			bqsrBam, recErr := Recalibrate(resolvedFasta, rgmdBam, sampleKnownSites, sampleLogPath, verbose)
			if recErr != nil {
				sl.Error("BQSR", "PROGRAM", bqsrStage, "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", recErr))
				jlog.Error("BQSR", "PROGRAM", bqsrStage, "SAMPLE", sampleName, "STATUS", fmt.Sprintf("FAILED - %v", recErr))
				addFailure(fmt.Sprintf("%s: %v", bqsrStage, recErr))
				return
			}

			sl.Info("BQSR", "PROGRAM", bqsrStage, "SAMPLE", sampleName, "STATUS", fmt.Sprintf("COMPLETED - BQSR BAM: %s", bqsrBam))
			jlog.Info("BQSR", "PROGRAM", bqsrStage, "SAMPLE", sampleName, "STATUS", "COMPLETED")
			color.Green("[%s] %s completed. Final BAM: %s\n", sampleName, bqsrStage, bqsrBam)
