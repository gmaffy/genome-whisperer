...
func Bowtie2Align(forwardPath string, reversePath string, referencePath string, sampleName string, libName string, threads int, verbose bool) error {
...
func RunAlignReads(referencePath string, forwardPath string, reversePath string, sePath string, sampleName string, libName string, outputDir string, threads int, aligner string, knownSites []string, bqsr bool, bootstrap bool, logFilePath string, preset string, gatkLogLevel string, verbose bool, outputFmt string) (string, error) {
	...
	if aligner == "bwa-mem2" {
		...
		if utils.StageHasCompleted(logged, "MARK_DUPLICATES", sampleName, "ALL") {
			...
			var mdupErr error
			if verbose {
				mdupErr = utils.RunBashCmdVerbose(cmd)
			} else {
				mdupErr = utils.RunBashCmd(cmd)
			}
			if mdupErr != nil {
		...
		if utils.StageHasCompleted(logged, "MARK_DUPLICATES", sampleName, "ALL") {
			...
			cmd := fmt.Sprintf(`gatk --java-options "-Xmx8G" MarkDuplicates -I %s -O %s -M %s --VERBOSITY %s`, sortedBam, rgmdBam, rgmdMetrics, gatkLogLevel)
			fmt.Printf("%s\n-----------------------------------------------\n\n", cmd)
		...
		if utils.StageHasCompleted(logged, "BOWTIE2", sampleName, "ALL") {
			...
			cmd := fmt.Sprintf(`gatk --java-options "-Xmx8G" BuildBamIndex -I %s -O %s`, sortedBam, sortedBai)
			var mErr error
			if verbose {
				mErr = utils.RunBashCmdVerbose(cmd)
			} else {
				mErr = utils.RunBashCmd(cmd)
			}
			if mErr != nil {
