...
func Stats(ref, bam string, verbose bool) error {
	cmdStr := fmt.Sprintf(`gatk CollectAlignmentSummaryMetrics -R %s -I %s -O %s --TMP_DIR %s`, ref, bam, strings.TrimSuffix(bam, ".bam")+"_alignment_metrics.txt", WorkTmpDir(bam))

	cmdStr2 := fmt.Sprintf(`gatk CollectInsertSizeMetrics -R %s -I %s -O %s -H %s --TMP_DIR %s`, ref, bam, strings.TrimSuffix(bam, ".bam")+"_insert_metrics.txt", strings.TrimSuffix(bam, ".bam")+"_insert_size_histogram.pdf", WorkTmpDir(bam))
