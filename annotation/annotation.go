...
func CreateCustomDb(ref string, prot string, cds string, species string, version string) error {
	// --------------------------------------------------- Get Config file ---------------------------------------- //
	snEffPath, err := exec.LookPath("snpEff")
	if err != nil {
		return fmt.Errorf("snpEff not found: %w", err)
	...
	fmt.Printf(" mkdir $snpEff/data/$Species$version",
	mkdir $snpEff/data/$Species$version

	echo "Copying reference and protein files .."
	echo "cp $Reference $snpEff/data/$Species$version/sequences.fa"
	cp $Reference $snpEff/data/$Species$version/sequences.fa
	echo "cp $proteins $snpEff/data/$Species$version/protein.fa"
	cp $proteins $snpEff/data/$Species$version/protein.fa
	echo "cp $cds $snpEff/data/$Species$version/cds.fa"
	cp $cds $snpEff/data/$Species$version/cds.fa
