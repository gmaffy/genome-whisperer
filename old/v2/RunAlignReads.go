...
func RunAlignReadsDir(dataDir string, species string, refVer string, refFasta string, genomesDir string, verbose bool, gatkLogLevel string, aligner string, quick bool, skipVer bool, bqsr bool, bootstrap bool, knownSites []string, threads int) {
	...
	for _, sample := range samples {
		...
		_, cramFiles := variants.FindBamOrVcfs(dataDirAbs, species, sampleName, refVer, "bams", "")
	...
	for _, rp := range validCleanReads {
		...
		go func(readPair samplePair) {
			...
			// Per-sample copy so bootstrap appends don't race across goroutines
			...
			if len(sampleKnownSites) == 0 && bootstrap {
				...
				newKS, ksErr := CreateKnownVariants(resolvedFasta, rgmdBam, "", gatkLogLevel, verbose)
				if ksErr != nil {
					addFailure(fmt.Sprintf("CreateKnownVariants: %v", ksErr))
					return
				}
			...
			bqsrBam, recErr := Recalibrate(resolvedFasta, rgmdBam, sampleKnownSites, "", verbose)
			if recErr != nil {
				addFailure(fmt.Sprintf("Recalibrate: %v", recErr))
				return
			}
			color.Green("[%s] BQSR complete. Final BAM: %s\n", sampleName, bqsrBam)
